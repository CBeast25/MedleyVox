...
# In this code, we use librispeech-dev as a validation set.
# For making 'same singer's different speech + speech + speech + speech + ...' validation set.
...
singing_data_list = glob.glob(f"{args.speech_data_root}\\*.wav")
...
with open(
    ".\\valid_regions_dict_speech_speech_same_speaker_n_srcs.json", "w"
) as json_file:

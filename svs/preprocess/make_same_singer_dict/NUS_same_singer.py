# make dict of NUS dataset's {same singer: [song_list]}

import os
import glob
import json

train_list = glob.glob("F:\\Stuff\\NUS\\*\\sing\\*.wav")

total_list = train_list

singer_list = []
for path in total_list:
    song_name = os.path.basename(path).replace(".wav", "")
    path_split = path.split("\\")
    singer_name = "NUS" + " - " + path_split[-3]
    singer_list.append(singer_name)

singer_list = sorted(list(set(singer_list)))

song_path_dict = {}

for singer in singer_list:
    song_path_dict[singer] = []
for path in total_list:
    song_name = os.path.basename(path).replace(".wav", "")
    path_split = path.split("\\")
    singer_name = "NUS" + " - " + path_split[-3]
    song_path_dict[singer_name] += [path_split[-3] + "_" + song_name]

with open(".\\same_singer_NUS.json", "w") as json_file:
    json.dump(song_path_dict, json_file, indent=0)
